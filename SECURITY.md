...
- **Merge-time pin gate** (`.github/workflows/quality-gates.yml`): the central
  `pin-check` reusable runs on every push and pull request.
...
> Some gates are pending org Actions allow-list additions and therefore not
> wired yet: **SAST** (CodeQL) and **SCA** (OSV-Scanner) need
> `github/codeql-action` permitted (and the OSV subpath
> `google/osv-scanner-action/*`); **Trivy** and **OpenSSF Scorecard** need
> `aquasecurity/trivy-action` and `ossf/scorecard-action`. Calling any of them
> before that startup-fails. The caller jobs are staged (commented) in
> `quality-gates.yml` and enable with a one-line allow-list change.
...
gh attestation verify ./_verify/artifact.tar \
  ...
  --signer-workflow attested-delivery/attested-delivery.github.io/.github/workflows/deploy.yml \
